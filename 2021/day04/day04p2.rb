...
class Board
        ...
        attr_accessor :isWinner

        def initialize()
            self.isWinner = false
            self.iValues = Array.new
        end
...
def isWinner(board, iDrawnNumbers)
    ...
    while (i < BOARD_SIZE)
        j = 0
        while (j < BOARD_SIZE)
            if (iDrawnNumbers.include?(board.iValues[i][j]))
...
def getScore(board, iDrawnNumbers)
    ...
    while (i < BOARD_SIZE)
        j = 0
        while (j < BOARD_SIZE)
            if (not iDrawnNumbers.include?(board.iValues[i][j]))
                
                iScore += board.iValues[i][j]
            end
...
def allBoardsCompleted(boards)
    allBoardsCompleted = true
